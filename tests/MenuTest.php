...
class MenuTest extends TestCase
{
    ...
    public function testCanRenameMenu()
    {
        ...
        $this->visitRoute('ezcp.menus.edit', ['id' => $menu->id])
             ->seeInField('name', $menu->name)
             ->type('new_admin', 'name')
             ->seeInElement('button', __('ezcp::generic.save'))
             ->press(__('ezcp::generic.save'))
             ->seePageIs(route('ezcp.menus.index'))
             ->seeInDatabase('menus', [
                'id'   => $menu->id,
                'name' => 'new_admin',
             ]);
    ...
    {
        $menu = Menu::where('name', '=', 'admin')->first();
        $this->delete(route('ezcp.menus.item.destroy', [
            'menu' => $menu->id,
    ...
    {
        $menu = Menu::where('name', '=', 'admin')->first();
        $this->post(route('ezcp.menus.item.add', [
            'menu'    => $menu->id,
            'menu_id' => $menu->id,
            'type'    => 'url',
            'color'   => '#000000',
    ...
    public function testCanUpdateMenuItem()
    {
        ...
        $this->put(route('ezcp.menus.item.update', [
            'id'      => $item->id,
            'menu'    => $menu->id,
            'type'    => 'url',
            'color'   => '#000000',
    ...
    {
        $menu = Menu::where('name', '=', 'admin')->first();
        $response = $this->call('GET', route('ezcp.menus.builder', ['menu' => $menu->id]));
