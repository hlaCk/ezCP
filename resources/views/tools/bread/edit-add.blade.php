...
@if (isset($dataType->id))
    @section('page_title', __('ezcp::bread.edit_bread_for_table', ['table' => $dataType->name]))
@else
    @section('page_title', __('ezcp::bread.create_bread_for_table', ['table' => $table]))
@endif
...
@section('page_header')
    <div class="page-title">
        <i class="ezcp-data"></i>
        @if (isset($dataType->id))
            {{ __('ezcp::bread.edit_bread_for_table', ['table' => $dataType->name]) }}
        @else
            {{ __('ezcp::bread.create_bread_for_table', ['table' => $table]) }}
        @endif
    </div>
    @php
    ...
    @endphp
    @include('ezcp::multilingual.language-selector')
@stop

@section('breadcrumbs')
<ol class="breadcrumb hidden-xs">
    <li class="active">
        <a href="{{ route('ezcp.dashboard')}}"><i class="ezcp-boat"></i> {{ __('ezcp::generic.dashboard') }}</a>
    </li>
    <li class="active">
        <a href="{{ route('ezcp.bread.index') }}">
            {{ __('ezcp::generic.bread') }}
        </a>
    </li>
    <li class="active">
        @if(isset($dataType->id))
        <a href="{{ route('ezcp.bread.edit', $table) }}">
            {{ $dataType->display_name_singular }}
        </a>
        @else
        <a href="{{ route('ezcp.bread.create', ['name' => $table]) }}">
            {{ $display_name }}
        </a>
        @endif
    </li>
    <li>
        {{ isset($dataType->id) ? __('ezcp::generic.edit') : __('ezcp::generic.add') }}
    </li>
</ol>
@endsection

@section('content')
    <div class="page-content container-fluid" id="ezcpBreadEditAdd">
        <div class="row">
            <div class="col-md-12">

                <form action="@if(isset($dataType->id)){{ route('ezcp.bread.update', $dataType->id) }}@else{{ route('ezcp.bread.store') }}@endif"
                ...
                @endif
                    ...
                    <div class="panel panel-primary panel-bordered">
                        ...
                        <div class="panel-heading">
                            <h3 class="panel-title panel-icon"><i class="ezcp-bread"></i> {{ ucfirst($table) }} {{ __('ezcp::bread.bread_info') }}</h3>
                            <div class="panel-actions">
                                <a class="panel-action ezcp-angle-up" data-toggle="panel-collapse" aria-hidden="true"></a>
                            </div>
                        </div>
                        ...
                        <div class="panel-body">
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="name">{{ __('ezcp::database.table_name') }}</label>
                            ...
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="display_name_singular">{{ __('ezcp::bread.display_name_singular') }}</label>
                                    @if($isModelTranslatable)
                                        @include('ezcp::multilingual.input-hidden', [
                                    ...
                                    <input type="text" class="form-control"
                                           ...
                                           placeholder="{{ __('ezcp::bread.display_name_singular') }}"
                                           value="{{ $dataType->display_name_singular ?? $display_name }}">
                                </div>
                                <div class="col-md-6 form-group">
                                    <label for="display_name_plural">{{ __('ezcp::bread.display_name_plural') }}</label>
                                    @if($isModelTranslatable)
                                        @include('ezcp::multilingual.input-hidden', [
                                            'isModelTranslatable' => true,
                                    ...
                                    <input type="text" class="form-control"
                                           ...
                                           placeholder="{{ __('ezcp::bread.display_name_plural') }}"
                                           value="{{ $dataType->display_name_plural ?? $display_name_plural }}">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="slug">{{ __('ezcp::bread.url_slug') }}</label>
                                    <input type="text" class="form-control" name="slug" placeholder="{{ __('ezcp::bread.url_slug_ph') }}"
                                           value="{{ $dataType->slug ?? $slug }}">
                                </div>
                                <div class="col-md-6 form-group">
                                    <label for="icon">{{ __('ezcp::bread.icon_hint') }} <a
                                                href="{{ route('ezcp.compass.index', [], false) }}#fonts"
                                                target="_blank">{{ __('ezcp::bread.icon_hint2') }}</a></label>
                                    <input type="text" class="form-control" name="icon"
                                           placeholder="{{ __('ezcp::bread.icon_class') }}"
                                           value="{{ $dataType->icon ?? '' }}">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="model_name">{{ __('ezcp::bread.model_name') }}</label>
                                    <span class="ezcp-question"
                                        aria-hidden="true"
                                        data-toggle="tooltip"
                                        data-placement="right"
                                        title="{{ __('ezcp::bread.model_name_ph') }}"></span>
                                    <input type="text" class="form-control" name="model_name" placeholder="{{ __('ezcp::bread.model_class') }}"
                                           value="{{ $dataType->model_name ?? $model_name }}">
                                </div>
                                <div class="col-md-6 form-group">
                                    <label for="controller">{{ __('ezcp::bread.controller_name') }}</label>
                                    <span class="ezcp-question"
                                        aria-hidden="true"
                                        data-toggle="tooltip"
                                        data-placement="right"
                                        title="{{ __('ezcp::bread.controller_name_hint') }}"></span>
                                    <input type="text" class="form-control" name="controller" placeholder="{{ __('ezcp::bread.controller_name') }}"
                                           value="{{ $dataType->controller ?? '' }}">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="policy_name">{{ __('ezcp::bread.policy_name') }}</label>
                                    <span class="ezcp-question"
                                          aria-hidden="true"
                                          data-toggle="tooltip"
                                          data-placement="right"
                                          title="{{ __('ezcp::bread.policy_name_ph') }}"></span>
                                    <input type="text" class="form-control" name="policy_name" placeholder="{{ __('ezcp::bread.policy_class') }}"
                                           value="{{ $dataType->policy_name ?? '' }}">
                                </div>
                                <div class="col-md-3 form-group">
                                    <label for="generate_permissions">{{ __('ezcp::bread.generate_permissions') }}</label><br>
                                    ...
                                    <input type="checkbox" name="generate_permissions" class="toggleswitch" data-on="{{ __('ezcp::generic.yes') }}" data-off="{{ __('ezcp::generic.no') }}"
                                           @if($checked) checked @endif>
                                </div>
                                <div class="col-md-3 form-group">
                                    <label for="server_side">{{ __('ezcp::bread.server_pagination') }}</label><br>
                                    <?php $checked = (isset($dataType->server_side) && $dataType->server_side == 1) ? true : (isset($server_side) && $server_side) ? true : false; ?>
                                    <input type="checkbox" name="server_side" class="toggleswitch" data-on="{{ __('ezcp::generic.yes') }}" data-off="{{ __('ezcp::generic.no') }}"
                                           @if($checked) checked @endif>
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-3 form-group">
                                    <label for="order_column">{{ __('ezcp::bread.order_column') }}</label>
                                    <span class="ezcp-question"
                                          aria-hidden="true"
                                          data-toggle="tooltip"
                                          data-placement="right"
                                          title="{{ __('ezcp::bread.order_column_ph') }}"></span>
                                    <select name="order_column" class="select2 form-control">
                                          <option value="">-- {{ __('ezcp::generic.none') }} --</option>
                                ...
                                <div class="col-md-3 form-group">
                                    <label for="order_display_column">{{ __('ezcp::bread.order_ident_column') }}</label>
                                    <span class="ezcp-question"
                                          aria-hidden="true"
                                          data-toggle="tooltip"
                                          data-placement="right"
                                          title="{{ __('ezcp::bread.order_ident_column_ph') }}"></span>
                                ...
                                <div class="col-md-3 form-group">
                                    <label for="order_direction">{{ __('ezcp::bread.order_direction') }}</label>
                                    <select name="order_direction" class="select2 form-control">
                                        <option value="asc" @if(isset($dataType) && $dataType->order_direction == 'asc') selected @endif>
                                            {{ __('ezcp::generic.ascending') }}
                                        </option>
                                        <option value="desc" @if(isset($dataType) && $dataType->order_direction == 'desc') selected @endif>
                                            {{ __('ezcp::generic.descending') }}
                                        </option>
                                    </select>
                                </div>
                                <div class="col-md-3 form-group">
                                    <label for="default_search_key">{{ __('ezcp::bread.default_search_key') }}</label>
                                    <span class="ezcp-question"
                                          aria-hidden="true"
                                          data-toggle="tooltip"
                                          data-placement="right"
                                          title="{{ __('ezcp::bread.default_search_key_ph') }}"></span>
                            ...
                            <div class="row clearfix">
                                @if (isset($scopes) && isset($dataType))
                                    <div class="col-md-3 form-group">
                                        <label for="scope">{{ __('ezcp::bread.scope') }}</label>
                                ...
                                <div class="col-md-9 form-group">
                                    <label for="description">{{ __('ezcp::bread.description') }}</label>
                                    <textarea class="form-control" name="description"
                                              placeholder="{{ __('ezcp::bread.description') }}"
                    ...
                    <div class="panel panel-primary panel-bordered">
                        <div class="panel-heading">
                            <h3 class="panel-title panel-icon"><i class="ezcp-window-list"></i> {{ __('ezcp::bread.edit_rows', ['table' => $table]) }}:</h3>
                            <div class="panel-actions">
                                <a class="panel-action ezcp-angle-up" data-toggle="panel-collapse" aria-hidden="true"></a>
                            </div>
                        </div>
                        ...
                        <div class="panel-body">
                            <div class="row fake-table-hd">
                                <div class="col-xs-2">{{ __('ezcp::database.field') }}</div>
                                <div class="col-xs-2">{{ __('ezcp::database.visibility') }}</div>
                                <div class="col-xs-2">{{ __('ezcp::database.input_type') }}</div>
                                <div class="col-xs-2">{{ __('ezcp::bread.display_name') }}</div>
                                <div class="col-xs-4">{{ __('ezcp::database.optional_details') }}</div>
                            </div>
                            ...
                            @foreach($fieldOptions as $data)
                                ...
                                @if(isset($dataType->id))
                                    <?php $dataRow = ezCP::model('DataRow')->where('data_type_id', '=',
                                ...
                                <div class="row row-dd">
                                    <div class="col-xs-2">
                                        ...
                                        <strong>{{ __('ezcp::database.type') }}:</strong> <span>{{ $data['type'] }}</span><br/>
                                        <strong>{{ __('ezcp::database.key') }}:</strong> <span>{{ $data['key'] }}</span><br/>
                                        <strong>{{ __('ezcp::generic.required') }}:</strong>
                                        @if($data['null'] == "NO")
                                            <span>{{ __('ezcp::generic.yes') }}</span>
                                            <input type="hidden" value="1" name="field_required_{{ $data['field'] }}"
                                                   checked="checked">
                                        @else
                                            <span>{{ __('ezcp::generic.no') }}</span>
                                            <input type="hidden" value="0" name="field_required_{{ $data['field'] }}">
                                        @endif
                                        <div class="handler ezcp-handle"></div>
                                    ...
                                    <div class="col-xs-2">
                                        <input type="checkbox"
                                               ...
                                               @elseif(!isset($dataRow->browse))
                                                    {{ 'checked="checked"' }}
                                               @endif>
                                        <label for="field_browse_{{ $data['field'] }}">{{ __('ezcp::generic.browse') }}</label><br/>
                                        <input type="checkbox"
                                        ...
                                        <label for="field_read_{{ $data['field'] }}">{{ __('ezcp::generic.read') }}</label><br/>
                                        <input type="checkbox"
                                        ...
                                        <label for="field_edit_{{ $data['field'] }}">{{ __('ezcp::generic.edit') }}</label><br/>
                                        <input type="checkbox"
                                            ...
                                            <label for="field_add_{{ $data['field'] }}">{{ __('ezcp::generic.add') }}</label><br/>
                                        <input type="checkbox"
                            ...
                            @if(isset($dataTypeRelationships))
                                @foreach($dataTypeRelationships as $relationship)
                                    @include('ezcp::tools.bread.relationship-partial', $relationship)
                                @endforeach
                            @endif
                        ...
                        <div class="panel-footer">
                             <div class="btn btn-new-relationship"><i class="ezcp-heart"></i> <span>
                             {{ __('ezcp::database.relationship.create') }}</span></div>
                        </div>
                    </div><!-- .panel -->

                    <button type="submit" class="btn pull-right btn-primary">{{ __('ezcp::generic.submit') }}</button>
...
@include('ezcp::tools.bread.relationship-new-modal')
...
@section('javascript')
    ...
    <script>
            ...
            $('textarea[data-editor]').each(function () {
                ...
                textarea.closest('form').on('submit', function (ev) {
                    if (window.invalidEditors.length) {
                        ...
                        toastr.error('{{ __('ezcp::json.invalid_message') }}', '{{ __('ezcp::json.validation_errors') }}', {"preventDuplicates": true, "preventOpenDuplicates": true});
                    } else {
       ...
       $(function () {
            ...
            $('.ezcp-relationship-details-btn').click(function(){
                $(this).toggleClass('open');
                if($(this).hasClass('open')){
                    $(this).parent().parent().find('.ezcp-relationship-details').slideDown();
                } else {
                    $(this).parent().parent().find('.ezcp-relationship-details').slideUp();
                }
            });
        ...
        function populateRowsFromTable(dropdown){
            ...
            if(tbl.length != 0){
                $.get('{{ route('ezcp.database.index') }}/' + tbl, function(data){
                    $(dropdown).empty();
