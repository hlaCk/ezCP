...
@section('page_title', __('ezcp::generic.viewing').' '.__('ezcp::generic.bread'))

@section('page_header')
    <h1 class="page-title">
        <i class="ezcp-bread"></i> {{ __('ezcp::generic.bread') }}
    </h1>
@stop
                ...
                <table class="table table-striped database-tables">
                    <thead>
                        <tr>
                            ...
                            <th style="text-align:right">{{ __('ezcp::bread.bread_crud_actions') }}</th>
                        </tr>
                    </thead>
                            ...
                            @if($table->dataTypeId)
                                <a href="{{ route('ezcp.' . $table->slug . '.index') }}"
                                   class="btn btn-warning btn-sm browse_bread" style="margin-right: 0;">
                                    <i class="ezcp-plus"></i> {{ __('ezcp::generic.browse') }}
                                </a>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="ezcp-trash"></i>  {!! __('ezcp::bread.delete_bread_quest', ['table' => '<span id="delete_builder_name"></span>']) !!}</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('ezcp.bread.delete', ['id' => null]) }}" id="delete_builder_form" method="POST">
                        {{ method_field('DELETE') }}
