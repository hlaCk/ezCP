...
<div class="panel panel-bordered">
    <div class="panel-body">
        ...
        @else
            <div class="col-md-6">
        @endif
                <label for="name">{{ __('ezcp::database.table_name') }}</label><br>
                <input v-model.trim="table.name" type="text" class="form-control" placeholder="{{ __('ezcp::database.table_name') }}" required pattern="{{ $db->identifierRegex }}">
            </div>

        @if($db->action == 'create')
            <div class="col-md-3 col-sm-4 col-xs-6">
                <label for="create_model">{{ __('ezcp::database.create_model_table') }}</label><br>
                <input type="checkbox" name="create_model" data-toggle="toggle"
                       data-on="{{ __('ezcp::generic.yes_please') }}" data-off="{{ __('ezcp::generic.no_thanks') }}">
            </div>
            {{--
                Hide migration button until feature is available.
                 <div class="col-md-3 col-sm-4 col-xs-6">
                    <label for="create_migration">{{ __('ezcp::database.create_migration') }}</label><br>
        ...
        <div v-if="compositeIndexes.length" v-once class="alert alert-danger">
            <p>{{ __('ezcp::database.no_composites_warning') }}</p>
        </div>

        <div id="alertsContainer"></div>

        <template v-if="tableHasColumns">
            <p>{{ __('ezcp::database.table_columns') }}</p>

            <table class="table table-bordered" style="width:100%;">
                <thead>
                <tr>
                    <th>{{ __('ezcp::generic.name') }}</th>
                    <th>{{ __('ezcp::generic.type') }}</th>
                    <th>{{ __('ezcp::generic.length') }}</th>
                    <th>{{ __('ezcp::generic.not_null') }}</th>
                    <th>{{ __('ezcp::generic.unsigned') }}</th>
                    <th>{{ __('ezcp::generic.auto_increment') }}</th>
                    <th>{{ __('ezcp::generic.index') }}</th>
                    <th>{{ __('ezcp::generic.default') }}</th>
                    <th></th>
                </tr>
                </thead>
                <tbody>
        ...
        <div v-else>
          <p>{{ __('ezcp::database.table_no_columns') }}</p>
        </div>
    ...
    <div class="panel-footer">
        <input type="submit" class="btn btn-primary pull-right"
               value="@if($db->action == 'update'){{ __('ezcp::database.update_table') }}@else{{ __('ezcp::database.create_new_table') }}@endif"
...
@include('ezcp::tools.database.vue-components.database-column')
@include('ezcp::tools.database.vue-components.database-table-helper-buttons')
        ...
        methods: {
            addColumn(column) {
                ...
                if (column.name && this.hasColumn(column.name)) {
                    return toastr.error("{{ __('ezcp::database.column') }} " + column.name + " {{ __('ezcp::database.already_exists') }}");
                }
            ...
            renameColumn(column) {
                ...
                if ((existingColumn = this.getColumn(newName)) && (existingColumn !== column)) {
                    return toastr.error("{{ __('ezcp::database.column') }} " + newName + " {{ __('ezcp::database.already_exists') }}");
                }
            ...
            addIndex(index) {
                if (index.type == 'PRIMARY') {
                    if (this.table.primaryKeyName) {
                        return toastr.error("{{ __('ezcp::database.table_has_index') }}");
                    }
