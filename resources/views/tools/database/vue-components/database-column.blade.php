...
<tr class="newTableRow">
    ...
    <td>
        <select :value="index.type" @change="onIndexTypeChange"
                ...
                class="form-control">
            <option value=""></option>
            <option value="INDEX">{{ __('ezcp::database.index') }}</option>
            <option value="UNIQUE">{{ __('ezcp::database.unique') }}</option>
            <option value="PRIMARY">{{ __('ezcp::database.primary') }}</option>
        </select>
        <small v-if="column.composite" v-once>{{ __('ezcp::database.composite_warning') }}</small>
    </td>
    ...
    <td>
        <div class="btn btn-danger delete-row" @click="deleteColumn"><i class="ezcp-trash"></i></div>
    </td>
...
@include('ezcp::tools.database.vue-components.database-types')
@include('ezcp::tools.database.vue-components.database-column-default')
    ...
    Vue.component('database-column', {
        ...
        methods: {
            ...
            onIndexTypeChange(event) {
                if (this.column.name == '') {
                    return toastr.error("{{ __('ezcp::database.name_warning') }}");
                }
