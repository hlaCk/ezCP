@extends('ezcp::master')
@if($db->action == 'update')
    @section('page_title', __('ezcp::database.editing_table', ['table' => $db->table->name]))
@else
    @section('page_title', __('ezcp::database.create_new_table'))
@endif
...
@section('page_header')
    <h1 class="page-title">
        <i class="ezcp-data"></i>
        @if($db->action == 'update')
            {{ __('ezcp::database.editing_table', ['table' => $db->table->name]) }}
        @else
            {{ __('ezcp::database.create_new_table') }}
        @endif
    </h1>
@stop
...
<ol class="breadcrumb hidden-xs">
    ...
    <li>
        <a href="{{ route('ezcp.database.index') }}">
            {{ __('ezcp::generic.database') }}
        </a>
    </li>

    @if($db->action == 'update')
    <li class="active">{{ __('ezcp::generic.edit') }}</li>
    <li class="active">{{ $db->table->name }}</li>
    @else
    <li class="active">{{ __('ezcp::generic.add') }}</li>
    @endif
</ol>
@endsection
...
@section('javascript')
    @include('ezcp::tools.database.vue-components.database-table-editor')
