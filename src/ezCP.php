...
use hlaCk\ezCP\Actions\DeleteAction;
use hlaCk\ezCP\Actions\EditAction;
use hlaCk\ezCP\Actions\RestoreAction;
use hlaCk\ezCP\Actions\ViewAction;
use hlaCk\ezCP\Events\AlertsCollection;
use hlaCk\ezCP\FormFields\After\HandlerInterface as AfterHandlerInterface;
use hlaCk\ezCP\FormFields\HandlerInterface;
...
use hlaCk\ezCP\Models\Translation;
...
class ezCP
{
    ...
    {
        $widgetClasses = config('ezcp.dashboard.widgets');
        $dimmers = Widget::group('ezcp::dimmers');
    ...
    {
        $globalCache = config('ezcp.settings.cache', false);
    ...
    {
        if (!empty($file)) {
            return str_replace('\\', '/', Storage::disk(config('ezcp.storage.disk'))->url($file));
        }
    ...
    {
        require __DIR__.'/../routes/ezcp.php';
    }
    ...
    protected function findVersion()
    {
        ...
        if ($this->filesystem->exists(base_path('composer.lock'))) {
            ...
            // Loop through all the packages and get the version of ezcp
            foreach ($file->packages as $package) {
                if ($package->name == 'hlack/ezcp') {
    ...
    {
        if (!config('ezcp.multilingual.enabled')) {
            return false;
        }
