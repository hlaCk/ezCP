...
class ezCPMediaController extends Controller
{
    /** @var string */
    private $filesystem;
    ...
    {
        $this->filesystem = config('ezcp.storage.disk');
    }
    ...
    public function index()
    {
        ...
        return ezCP::view('ezcp::media.index');
    }
    ...
    public function files(Request $request)
    {
        ...
        foreach ($storageItems as $item) {
            if ($item['type'] == 'dir') {
                ...
                if (empty(pathinfo($item['path'], PATHINFO_FILENAME)) && !config('ezcp.hidden_files')) {
                    continue;
                }
                $files[] = [
                    'name'          => $item['basename'],
    ...
    public function new_folder(Request $request)
    {
        ...
        if (Storage::disk($this->filesystem)->exists($new_folder)) {
            $error = __('ezcp::media.folder_exists_already');
        } elseif (Storage::disk($this->filesystem)->makeDirectory($new_folder)) {
            $success = true;
        } else {
            $error = __('ezcp::media.error_creating_dir');
        }
        ...
        foreach ($request->get('files') as $file) {
            ...
            if ($file['type'] == 'folder') {
                if (!Storage::disk($this->filesystem)->deleteDirectory($file_path)) {
                    $error = __('ezcp::media.error_deleting_folder');
                    $success = false;
                }
            } elseif (!Storage::disk($this->filesystem)->delete($file_path)) {
                $error = __('ezcp::media.error_deleting_file');
                $success = false;
            }
        }
    ...
    public function rename(Request $request)
    {
        ...
        if (!Storage::disk($this->filesystem)->exists("{$location}/{$newFilename}")) {
            if (Storage::disk($this->filesystem)->move("{$location}/{$filename}", "{$location}/{$newFilename}")) {
                ...
                $error = __('ezcp::media.error_moving');
            }
        } else {
            $error = __('ezcp::media.error_may_exist');
        }
    ...
    public function upload(Request $request)
    {
        ...
        try {
            ...
            $allowedMimeTypes = config('ezcp.media.allowed_mimetypes', '*');
            if ($allowedMimeTypes != '*' && (is_array($allowedMimeTypes) && !in_array($request->file->getMimeType(), $allowedMimeTypes))) {
                throw new Exception(__('ezcp::generic.mimetype_not_allowed'));
            }
            ...
            $success = true;
            $message = __('ezcp::media.success_uploaded_file');
    ...
    public function remove(Request $request)
    {
        ...
        try {
            ...
            if (!isset($data->{$field})) {
                throw new Exception(__('ezcp::generic.field_does_not_exist'), 400);
            }

            if (@json_decode($multi)) {
                // Check if valid json
                if (is_null(@json_decode($data->{$field}))) {
                    throw new Exception(__('ezcp::json.invalid'), 500);
                }
                ...
                if (is_null($key) || $key === false) {
                    throw new Exception(__('ezcp::media.file_does_not_exist'), 400);
                }
            ...
            return response()->json([
               'data' => [
                   'status'  => 200,
                   'message' => __('ezcp::media.file_removed'),
               ],
            ]);
        } catch (Exception $e) {
            $code = 500;
            $message = __('ezcp::generic.internal_error');
    ...
    public function crop(Request $request)
    {
        ...
        try {
            ...
            $success = true;
            $message = __('ezcp::media.success_crop_image');
        } catch (Exception $e) {
            $success = false;
            $message = $e->getMessage();
        }
