...
use hlaCk\ezCP\Http\Controllers\ContentTypes\Checkbox;
use hlaCk\ezCP\Http\Controllers\ContentTypes\Coordinates;
use hlaCk\ezCP\Http\Controllers\ContentTypes\File;
use hlaCk\ezCP\Http\Controllers\ContentTypes\Image as ContentImage;
use hlaCk\ezCP\Http\Controllers\ContentTypes\MultipleCheckbox;
use hlaCk\ezCP\Http\Controllers\ContentTypes\MultipleImage;
use hlaCk\ezCP\Http\Controllers\ContentTypes\Password;
use hlaCk\ezCP\Http\Controllers\ContentTypes\Relationship;
use hlaCk\ezCP\Http\Controllers\ContentTypes\SelectMultiple;
use hlaCk\ezCP\Http\Controllers\ContentTypes\Text;
use hlaCk\ezCP\Http\Controllers\ContentTypes\Timestamp;
use hlaCk\ezCP\Traits\AlertsMessages;
use Validator;
...
abstract class Controller extends BaseController
{
    ...
    public function insertUpdateData($request, $slug, $rows, $data)
    {
        ...
        if ($request->session()->has($slug.'_path') || $request->session()->has($slug.'_uuid')) {
            ...
            $data->save();
            if ($old_path != $new_path && !Storage::disk(config('ezcp.storage.disk'))->exists($new_path)) {
                $request->session()->forget([$slug.'_path', $slug.'_uuid']);
                Storage::disk(config('ezcp.storage.disk'))->move($old_path, $new_path);
                Storage::disk(config('ezcp.storage.disk'))->deleteDirectory($folder_path);
            }
        }
    ...
    {
        if (Storage::disk(config('ezcp.storage.disk'))->exists($path)) {
            Storage::disk(config('ezcp.storage.disk'))->delete($path);
