...
use hlaCk\ezCP\Database\DatabaseUpdater;
use hlaCk\ezCP\Database\Schema\Column;
use hlaCk\ezCP\Database\Schema\Identifier;
...
class ezCPDatabaseController extends Controller
{
    public function index()
    {
        ...
        return ezCP::view('ezcp::tools.database.index')->with(compact('dataTypes', 'tables'));
    }
    ...
    public function create()
    {
        ...
        return ezCP::view('ezcp::tools.database.edit-add', compact('db'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (isset($request->create_model) && $request->create_model == 'on') {
                ...
                Artisan::call('ezcp:make:model', $params);
            ...
            return redirect()
               ...
               ->with($this->alertSuccess(__('ezcp::database.success_create_table', ['table' => $table->name])));
        } catch (Exception $e) {
    ...
    public function edit($table)
    {
        ...
        if (!SchemaManager::tableExists($table)) {
            return redirect()
                ->route('ezcp.database.index')
                ->with($this->alertError(__('ezcp::database.edit_table_not_exist')));
        }
    ...
    protected function prepareDbManager($action, $table = '')
    {
        ...
        if ($action == 'update') {
            ...
            $db->formAction = route('ezcp.database.update', $table);
        } else {
            ...
            $db->formAction = route('ezcp.database.store');
        }
    ...
    public function show($table)
    {
        ...
        $model_name = ezCP::model('DataType')->where('name', $table)->pluck('model_name')->first();
        if (isset($model_name)) {
    ...
    public function destroy($table)
    {
        ...
        try {
            ...
            return redirect()
                ->route('ezcp.database.index')
                ->with($this->alertSuccess(__('ezcp::database.success_delete_table', ['table' => $table])));
        } catch (Exception $e) {
