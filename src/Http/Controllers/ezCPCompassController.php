...
class ezCPCompassController extends Controller
{
    protected $request;
    ...
    public function index(Request $request)
    {
        ...
        if (!\App::environment('local') && !config('ezcp.compass_in_production', false)) {
        ...
        if ($this->request->input('download')) {
            ...
            return $this->redirect($this->request->url().'?logs=true')->with([
                'message'    => __('ezcp::compass.commands.delete_success').' '.base64_decode($this->request->input('del')),
                'alert-type' => 'success',
                ]);
            ...
            return $this->redirect($this->request->url().'?logs=true')->with([
                'message'    => __('ezcp::compass.commands.delete_all_success'),
                'alert-type' => 'success',
                ]);
        }
        ...
        return view('ezcp::compass.index', compact('logs', 'files', 'current_file', 'active_tab', 'commands', 'artisan_output'))->with($message);
    }
