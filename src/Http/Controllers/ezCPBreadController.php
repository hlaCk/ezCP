...
use ReflectionClass;
use hlaCk\ezCP\Database\Schema\Column;
use hlaCk\ezCP\Database\Schema\SchemaManager;
...
class ezCPBreadController extends Controller
{
    public function index()
    {
        $this->authorize('browse_bread');

        $dataTypes = ezCP::model('DataType')->select('id', 'name', 'slug')->get()->keyBy('name')->toArray();
        ...
        return ezCP::view('ezcp::tools.bread.index')->with(compact('dataTypes', 'tables'));
    }
    ...
    public function create(Request $request, $table)
    {
        ...
        $dataType = ezCP::model('DataType')->whereName($table)->first();
        ...
        return ezCP::view('ezcp::tools.bread.edit-add', $data);
    }

    private function prepopulateBreadInfo($table)
    {
        $displayName = Str::singular(implode(' ', explode('_', Str::title($table))));
        $modelNamespace = config('ezcp.models.namespace', app()->getNamespace());
    ...
    public function store(Request $request)
    {
        ...
        try {
            $dataType = ezCP::model('DataType');
            $res = $dataType->updateDataType($request->all(), true);
            $data = $res
                ? $this->alertSuccess(__('ezcp::bread.success_created_bread'))
                : $this->alertError(__('ezcp::bread.error_creating_bread'));
            if ($res) {
                event(new BreadAdded($dataType, $data));
            }

            return redirect()->route('ezcp.bread.index')->with($data);
        } catch (Exception $e) {
            return redirect()->route('ezcp.bread.index')->with($this->alertException($e, 'Saving Failed'));
        }
    }
    ...
    public function edit($table)
    {
        ...
        $dataTypeRelationships = ezCP::model('DataRow')->where('data_type_id', '=', $dataType->id)->where('type', '=', 'relationship')->get();
        $scopes = [];
        if ($dataType->model_name != '') {
            $scopes = $this->getModelScopes($dataType->model_name);
        }

        return ezCP::view('ezcp::tools.bread.edit-add', compact('dataType', 'fieldOptions', 'isModelTranslatable', 'tables', 'dataTypeRelationships', 'scopes'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        /* @var \hlaCk\ezCP\Models\DataType $dataType */
        try {
            $dataType = ezCP::model('DataType')->find($id);
            ...
            $res = $dataType->updateDataType($request->all(), true);
            $data = $res
                ? $this->alertSuccess(__('ezcp::bread.success_update_bread', ['datatype' => $dataType->name]))
                : $this->alertError(__('ezcp::bread.error_updating_bread'));
            if ($res) {
            ...
            return back()->with($this->alertException($e, __('ezcp::generic.update_failed')));
        }
    }
    ...
    public function destroy($id)
    {
        ...
        $res = ezCP::model('DataType')->destroy($id);
        $data = $res
            ? $this->alertSuccess(__('ezcp::bread.success_remove_bread', ['datatype' => $dataType->name]))
            : $this->alertError(__('ezcp::bread.error_updating_bread'));
        if ($res) {
        ...
        if (!is_null($dataType)) {
            ezCP::model('Permission')->removeFrom($dataType->name);
        }
    ...
    public function addRelationship(Request $request)
    {
        ...
        try {
            ...
            $className = ezCP::modelClass('DataRow');
            ...
            $newRow->order = intval(ezCP::model('DataType')->find($request->data_type_id)->lastRow()->order) + 1;
    ...
    private function getRelationshipField($request)
    {
        ...
        $dataType = ezCP::model('DataType')->find($request->data_type_id);
        ...
        $existingRow = ezCP::model('DataRow')->where('field', '=', $relationshipField)->first();
        $index = 1;
