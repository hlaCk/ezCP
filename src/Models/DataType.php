...
class DataType extends Model
{
    ...
    {
        return $this->hasMany(ezCP::modelClass('DataRow'))->orderBy('order');
    }
    ...
    {
        return $this->hasMany(ezCP::modelClass('DataRow'))->orderBy('order', 'DESC')->first();
    }
        ...
        try {
            ...
            if ($this->fill($requestData)->save()) {
                ...
                foreach ($fields as $field) {
                    ...
                    if (!$dataRow->save()) {
                        throw new \Exception(__('ezcp::database.field_safe_failed', ['field' => $field]));
                    }
                }
                ...
                if ($this->generate_permissions) {
                    ezCP::model('Permission')->generateFor($this->name);
                }
