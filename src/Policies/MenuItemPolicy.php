...
namespace hlaCk\ezCP\Policies;

use hlaCk\ezCP\Contracts\User;
...
class MenuItemPolicy extends BasePolicy
{
    ...
    protected function checkPermission(User $user, $model, $action)
    {
        if (self::$permissions == null) {
            self::$permissions = ezCP::model('Permission')->all();
        }

        if (self::$datatypes == null) {
            self::$datatypes = ezCP::model('DataType')::all()->keyBy('slug');
        }

        $regex = str_replace('/', '\/', preg_quote(route('ezcp.dashboard')));
        ...
        if ($slug == '') {
            $slug = 'admin';
        } elseif ($slug == 'compass' && !\App::environment('local') && !config('ezcp.compass_in_production', false)) {
            return false;
        }
